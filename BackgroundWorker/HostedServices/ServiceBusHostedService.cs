...
namespace BackgroundWorker.HostedServices
{
    public class ServiceBusHostedService : IHostedService
    {
        ...
        private async Task<(Dictionary<string, object>? ModifiedProperties, bool Success)> ReceivedMessageFunc(ProcessMessageEventArgs args)
        {
            ...
            LogMessage($"Message has been received. Application properties: [{string.Join(',', applicationProperties.Select(i => $"{i.Key}={i.Value}"))}]");
            ...
            if (TryGetValueFromApplicationProperties( // TODO: can metadata expire?
                ...
                out var historyId))
            {
                LogMessage($"History Id: {historyId} has been found in application settings.");
            }
            else
            {
            ...
            try
            {
                await _notificationHub.SendAsync(
                    ...
                    arg6: null,
                    cancellationToken: _mainCancellationToken);
                LogMessage($"Signal R initialize request for historyId: {historyId} has been sent.");
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                LogMessage($"Long running request for historyId: {historyId} has been completed.");
            }
            catch (Exception ex)
            {
            ...
            {
                LogMessage($"Resulted link ({resultedLink}) from blob for historyId: {historyId} has been found.");
            }
            else
            {
                try
                {
                    ...
                    LogMessage($"Resulted link ({resultedLink}) from blob for historyId: {historyId} has been generated.");
                }
                catch (Exception ex)
                {
            ...
            {
                LogMessage($"HistoryId: {historyId} updated record has been found in application settings.");
            }
            else
            {
                try
                {
                    ...
                    LogMessage($"HistoryId: {historyId} record has been updated.");
                }
                catch (Exception ex)
                {
            ...
            try
            {
                await _notificationHub.SendAsync(
                    ...
                    arg6: completed,
                    cancellationToken: _mainCancellationToken);
                LogMessage($"Signal R completed request for historyId: {historyId} has been sent.");
            }
            catch (Exception ex)
            {
                LogMessage("SignalR sending has been failed", ex);
